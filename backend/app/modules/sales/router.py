...
def list_sales_invoices(
    ...
    return list_invoices(db, current_user.company_id, "SALES")
...
def read_sales_invoice(
    ...
    try:
        return get_invoice(db, current_user.company_id, invoice_id, "SALES")
...
def add_sales_invoice(
    ...
    try:
        return create_invoice(db, current_user.company_id, "SALES", payload, current_user.id)
    except InvoiceError as exc:
        raise _invoice_http_error(exc) from exc
...
def post_sales_invoice(
    ...
    try:
        return post_invoice(db, current_user.company_id, invoice_id, "SALES", current_user.id)
    except InvoiceError as exc:
        raise _invoice_http_error(exc) from exc
...
def cancel_sales_invoice(
    ...
    try:
        return cancel_invoice(db, current_user.company_id, invoice_id, "SALES", payload, current_user.id)
    except InvoiceError as exc:
        raise _invoice_http_error(exc) from exc
